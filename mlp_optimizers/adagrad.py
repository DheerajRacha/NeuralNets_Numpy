...
for epoch in range(num_epochs):
    ...
    for x_train,y_train in zip(X_train,Y_train):
        ...
        for j in range(2):
            ...
            vel_w[-j-1] = vel_w[-j-1] + grad_w*grad_w
            vel_b[-j-1] = vel_b[-j-1] + grad_b*grad_b

            theta_w = -(learning_rate/(epsilon + np.sqrt(vel_w[-j-1])))*grad_w
            theta_b = -(learning_rate/(epsilon + np.sqrt(vel_b[-j-1])))*grad_b
