...
s_adam_b = [np.zeros((y, 1)) for y in sizes[1:]]
r_adam_b = [np.zeros((y, 1)) for y in sizes[1:]]

weights = [np.random.normal(0,1,(y, x)) for x, y in zip(sizes[:-1], sizes[1:])]
s_adam_w = [np.zeros((y, x)) for x, y in zip(sizes[:-1], sizes[1:])]
r_adam_w = [np.zeros((y, x)) for x, y in zip(sizes[:-1], sizes[1:])]
...
for epoch in range(num_epochs):
    ...
    for x_train,y_train in zip(X_train,Y_train):
        ...
        for j in range(2):
            ...
            s_adam_w[-j-1] = rho1*s_adam_w[-j-1] + (1-rho1)*grad_w
            s_adam_b[-j-1] = rho1*s_adam_b[-j-1] + (1-rho1)*grad_b

            r_adam_w[-j-1] = rho2*r_adam_w[-j-1] + (1-rho2)*grad_w*grad_w
            r_adam_b[-j-1] = rho2*r_adam_b[-j-1] + (1-rho2)*grad_b*grad_b

            s_prime_w = s_adam_w[-j-1]/(1-np.power(rho1,count_t))
            s_prime_b = s_adam_b[-j-1]/(1-np.power(rho1,count_t))

            r_prime_w = r_adam_w[-j-1]/(1-np.power(rho2,count_t))
            r_prime_b = r_adam_b[-j-1]/(1-np.power(rho2,count_t))

            theta_w = -(learning_rate/np.sqrt(epsilon + r_prime_w))*s_prime_w
            theta_b = -(learning_rate/np.sqrt(epsilon + r_prime_b))*s_prime_b
