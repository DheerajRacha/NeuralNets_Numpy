...
for epoch in range(num_epochs):
    ...
    for x_train,y_train in zip(X_train,Y_train):
        ...
        for b, w in zip(biases, weights):
            ...
            zs.append(z)
            activation = mlp_functions.sigmoid(z)
        ...
        for j in range(3):
            if(j==0):
                ...
                grad_w, grad_b, prev_delta = mlp_functions.backprop(activations[-j-2],zs[-j-2],weights[-j-1],delta,"sigmoid",y_train)

            grad_b = np.reshape(grad_b , (grad_b.shape[0],1))
            weights[-j-1] -= learning_rate*grad_w
            biases[-j-1] -= learning_rate*grad_b
            delta=prev_delta
...
########### Evaluation ###############
    ...
    for x_test,y_test in zip(X_test,Y_test):
        activation_t = mlp_functions.feed_forward_network(x_test , weights , biases,"sigmoid")
    ...
    for x_train_t,y_train_t in zip(X_train,Y_train):
        activation = mlp_functions.feed_forward_network(x_train_t , weights , biases,"sigmoid")
